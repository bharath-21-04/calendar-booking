...
class CalendarClient:
    ...
    def __init__(self) -> None:
        ...
        credentials = google.oauth2.service_account.Credentials.from_service_account_info(
            settings.service_account_info, scopes=self.SCOPES
        )
    ...
    def _delete_event(self, event_id: str) -> None:
        self._session.delete(self._event_url(event_id)).raise_for_status()

    def _format_time(self, iso: str) -> str:
        try:
            dt = datetime.fromisoformat(iso.replace("Z", "+00:00")).astimezone(self._tz)
    ...
    def _create_booking_event(self, class_id: str, name: str, phone: str, start_iso: str, end_iso: str, timezone: str, class_title: str) -> str:
        body = {
            "summary": f"{name} | {class_title}",
            "description": f"Name:  {name}\nPhone: {phone}\nClass: {class_title}",
            "start": {"dateTime": start_iso, "timeZone": timezone},
            "end": {"dateTime": end_iso, "timeZone": timezone},
            "extendedProperties": {"private": {"type": "booking", "classId": class_id, "name": name, "phone": phone}},
        }
        event = self._create_event(body)
        log.info("calendar CREATE booking | id=%s  class=%s  name=%r  phone=%s", event["id"], class_id, name, phone)
        return event["id"]

    def _refresh_class_description(self, class_id: str, capacity: int) -> None:
        bookings = self._get_booking_events(class_id)
        names = [b.get("extendedProperties", {}).get("private", {}).get("name", "?") for b in bookings]
        booked = len(names)
        desc = f"Capacity: {capacity}  |  Booked: {booked}/{capacity}\nRoster: {', '.join(names) or '(none yet)'}"
        self._session.patch(self._event_url(class_id), json={"description": desc}).raise_for_status()

    def create_event(self, title: str, start_iso: str, end_iso: str, timezone: str, capacity: int, pre_bookings: list[dict] | None = None) -> str:
        body = {
            "summary": title,
            "start": {"dateTime": start_iso, "timeZone": timezone},
            "end": {"dateTime": end_iso, "timeZone": timezone},
            "extendedProperties": {"private": {"type": "class", "capacity": str(capacity)}},
        }
        event = self._create_event(body)
        class_id = event["id"]
        log.info("calendar CREATE class | id=%s  title=%r  capacity=%d", class_id, title, capacity)
        for booking in pre_bookings or []:
            self._create_booking_event(class_id, booking["name"], booking["phone"], start_iso, end_iso, timezone, title)
        return class_id
    ...
    def list_classes(self, date: str) -> list[ClassSlot]:
        ...
        booking_events = self._list_events([
            ("timeMin", t_min), ("timeMax", t_max),
            ("singleEvents", "true"), ("privateExtendedProperty", "type=booking"),
        ])
    ...
    def check_availability(self, class_id: str) -> int:
        ...
        capacity = int(ev.get("extendedProperties", {}).get("private", {}).get("capacity", self._default_capacity))
        return max(0, capacity - len(self._get_booking_events(class_id)))
    ...
    def book_class(self, class_id: str, name: str, phone: str) -> str:
        ...
        self._create_booking_event(class_id, name, phone, start_iso, end_iso, timezone, title)
    ...
    def cancel_booking(self, class_id: str, name: str, phone: str) -> str:
        bookings = self._get_booking_events(class_id)
        target = next(
            (b for b in bookings if b.get("extendedProperties", {}).get("private", {}).get("phone") == phone),
            None,
        )
        if target is None:
        ...
        log.info("calendar DELETE booking | id=%s  class=%s  phone=%s", target["id"], class_id, phone)

        class_ev = self._get_event(class_id)
        capacity = int(class_ev.get("extendedProperties", {}).get("private", {}).get("capacity", self._default_capacity))
        self._refresh_class_description(class_id, capacity)
    ...
    def reschedule_booking(self, old_class_id: str, new_class_id: str, name: str, phone: str) -> str:
